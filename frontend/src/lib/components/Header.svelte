...
<header class="h-16 border-b flex items-center px-3 md:px-6 gap-2 md:gap-3 shrink-0" style="background: var(--surface); border-color: var(--border);">
  ...
  <form onsubmit={handleSearch} class="flex-1 min-w-0 max-w-xl">
    <div class="relative">
      <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[var(--text-muted)] pointer-events-none" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      ...
      <input
        ...
        class="input pl-9 py-2 md:py-2.5"
      />
    </div>
  </form>
  ...
  <div class="flex items-center gap-0.5 md:gap-1 shrink-0">
    <div class="relative flex items-center justify-center">
      ...
      >
        ...
        {:else if activeBase === 'dark'}
          <svg class="w-4 h-4 md:w-5 md:h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>
        {:else}
          <svg class="w-4 h-4 md:w-5 md:h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg>
        {/if}
      </button>
    ...
    <a href="/opds" target="_blank" class="btn btn-ghost !p-2" title={$t('opds_feed')}>
      <svg class="w-4 h-4 md:w-5 md:h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 11a9 9 0 0 1 9 9"/><path d="M4 4a16 16 0 0 1 16 16"/><circle cx="5" cy="19" r="1"/></svg>
    </a>

    <button class="btn btn-ghost !p-2" onclick={logout} title={$t('logout')}>
      <svg class="w-4 h-4 md:w-5 md:h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>
    </button>
  </div>
</header>
