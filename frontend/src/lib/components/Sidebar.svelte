...
<aside class="w-60 shrink-0 border-r flex flex-col" style="background: var(--surface); border-color: var(--border);">
  ...
  <nav class="flex-1 p-3 space-y-0.5">
    {#each items as item}
      ...
      >
        {#if item.icon === 'home'}
          <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m3 9 9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
        ...
        {:else if item.icon === 'stats'}
          <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="20" x2="12" y2="10"/><line x1="18" y1="20" x2="18" y2="4"/><line x1="6" y1="20" x2="6" y2="16"/></svg>
        {:else}
  ...
  {#if $user}
    <div class="p-3 border-t" style="border-color: var(--border);">
      <div class="flex items-center gap-2.5 px-2 py-2">
        <div class="w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold" style="background: var(--accent); color: var(--accent-text);">
